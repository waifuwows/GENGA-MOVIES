...
Locate the line: `const CLOUD_BASE = 'http://localhost:8000';`.
Replace the string with your own server URL (make sure it doesn't end with a slash).
...
User Action: You click on a Manga, Music, or News item.
Frontend Request: Since the source is `manga`, the frontend sends the request to `http://localhost:8000/api/manga/...` (or directly to the News API).
Cloud Backend: The hosted backend receives the request.
Provider Fetch: The Cloud backend then talks to the actual provider (like Consumet or GaanaPy).
Data Return: The data travels back to your browser.
...
npm run dev
```
Open a deep-link in the browser (Vite dev server also serves hash routes):
```text
